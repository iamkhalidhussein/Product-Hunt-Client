...
const Submissions = () => {
    ...
    const sections = {
        ...
        underReview: { label: 'Under Review', id: 'underReview' },
    ...
    return (
        ...
        <div className="flex mt-14 border items-start justify-between pl-5 pr-5 rounded-lg py-10 bg-white">
            ...
            />
            <div>
                <h3 className="text-[#1B2530] text-xl font-medium hover:text-[#686EF8] cursor-pointer pb-2">
                    Md Khalid
                </h3>
                <div className="flex items-center text-white bg-[#686EF8] justify-center rounded-3xl py-1 px-3">
                <CgEditBlackPoint className="text-xl mr-1" />
                <span className="mr-1">0</span>Points
                </div>
            </div>
            </div>
        ...
        <h2 className="text-[#1B2530] text-[36px] font-semibold py-9">Submitted Resources</h2>
        ...
        {Object.keys(sections).map((key) => (
            <div key={key} id={sections[key].id} className={activeSection === key ? "" : "hidden"}>
            <p>{sections[key].label}</p>
            <EmptyState />
            </div>
        ))}
...
const StatusButton = ({ id, label, isActive, onClick }) => {
    return (
        <button
            id={`${id}-btn`}
            className={`py-2.5 px-6 text-white rounded ${
